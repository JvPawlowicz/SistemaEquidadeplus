...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!ok) {
      return new Response(JSON.stringify({ error: 'Apenas administradores podem enviar convites.' }), {
        status: 403,
    ...
    const raw = data as { action_link?: string; properties?: { action_link?: string } };
    const link = raw?.action_link ?? raw?.properties?.action_link ?? null;
    return new Response(
      JSON.stringify({
        message: 'Convite enviado. O usuário receberá um e-mail com o link de ativação (se o SMTP estiver configurado).',
        link: link ?? undefined,
      }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
